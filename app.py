...
def update_order(order_id):
    ...
    if new_status not in ['pending', 'preparing', 'prepared', 'delivery agent assigned', 'on the way', 'delivered']:
    ...
    if order:
        ...
        # Check if the delivery agent is available
            delivery_agent = mongo.db.delivery_agents.find_one(
                {'delivery_agent_name': order['delivery_agent_name'], 'status': 'available'},
            )
            if delivery_agent:
                # Update order status to 'on the way' only if the delivery agent is available
        ...
        # Update order status to 'delivered' and make delivery agent available
            mongo.db.orders.update_one(
                {'_id': ObjectId(order_id), 'status': current_status},
                {'$set': {'status': new_status}}
            )
...
def assign_da():
    ...
    prepared_orders = mongo.db.orders.find({
    "status": "prepared",
    "restaurant_id": ObjectId(restaurant_id)
    })
